...
class DoctorSpecialListPage extends StatefulWidget {
  const DoctorSpecialListPage({super.key, required this.response});
...
class _DoctorSpecialListPageState extends State<DoctorSpecialListPage> {
  ...
  void initState() {
    ...
    context.read<DoctorBloc>().add(
      LoadDoctorSpecial(id: widget.response.specialization.id),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.response.specialization.name)),
      body: Padding(
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final doctor = state.response[index];
                          return ListTile(
